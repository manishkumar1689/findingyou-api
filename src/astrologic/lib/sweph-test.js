...
const { isNumeric, isInteger, validLocationParameter, validISODateString } = require("./validators");
const { calcAsync, calcUtAsync } = require('./sweph-async');
const { calcJulDate, calcJulDateFromParts } = require('./astro-dates');
...
const parseFunc = async (funcname, params) => {
  let data = { valid: false };
  switch (funcname) {
    case 'planet':
...
module.exports = { planetData, calcJulDateFromParts, calcAsync, calcData, getConstantVals, getFuncNames, parseFunc };