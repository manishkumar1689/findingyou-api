...
const { calcJulDate, jdToDateTime, calcAstroWeekDayIndex } = require('./astro-dates');
const { JyotishDay } = require('./models/jyotish-day');
const { IndianTime } = require('./models/indian-time');
const { riseTransAsync } = require('./sweph-async');
const { isNumeric } = require('./validators');
const config = require('../.config');

const matchTransData = async (inData, transType = 0, transKey = 'trans') => {
...
const calcTransition = async (datetime, geo, planetNum, showInput = true) => {
  const jd = calcJulDate(datetime);
  const data = await calcTransitionJd(jd, geo, planetNum, showInput, true);
  return { jd, ...data };
}

const calcTransitionJd = async (jd, geo, planetNum, showInput = true, showIcMc = false) => {
  let data = { valid: false };
  if (isNumeric(jd)) {
    let valid = false;
    let longitude = 0;
...
const toIndianTime = async (datetime, geo) => {
